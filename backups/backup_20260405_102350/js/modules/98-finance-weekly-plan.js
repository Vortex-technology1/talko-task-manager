...
function _render(root) {
  ...
  const kpi = `
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-bottom:14px;">
      ${[
        {l:_t(_t(window.t('дохідПлан'),window.t('доходПлан')),'Доход план'),   v:_fmt(tPlanInc,cur), c:'#22c55e', sub:'за горизонт'},
        {l:_t(_t(window.t('витратиПлан'),window.t('расходыПлан')),'Расходы план'), v:_fmt(tPlanExp,cur), c:'#ef4444', sub:'за горизонт'},
  ...
  const controls = `
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;align-items:center;">
      <button onclick="window._wpSave()" id="wpSaveBtn"
        ...
        ${_t(window.t('зберегтиПлан'),window.t('сохранитьПлан'))}
      </button>
    ...
    return `
      <div style="position:absolute;left:${x}px;top:20px;width:${COL_W}px;height:${TOTAL_H-20}px;
        ...
        <div style="position:absolute;bottom:${CF_H+28}px;left:2px;width:${bw}px;height:${Math.max(1,aIncH)}px;
          ...
          title="${_t(window.t('дохідФакт'),window.t('доходФакт'))}: ${_fmt(w.aInc,currency)}"></div>` : ''}
        ...
        <div style="position:absolute;bottom:${CF_H+28}px;left:${halfW+2}px;width:${bw}px;height:${Math.max(1,aExpH)}px;
          ...
          title="${_t(window.t('витратиФакт'),window.t('расходыФакт'))}: ${_fmt(w.aExp,currency)}"></div>` : ''}
        ...
        <div style="position:absolute;bottom:${26 + cfZeroY - cfY - 4}px;left:50%;transform:translateX(-50%);
          ...
          title="${_t(window.t('залишок'),window.t('остаток'))}: ${_fmt(w.cfEnd,currency)}"></div>
...
window._wpSave = async function() {
  ...
  if (btn) { btn.disabled = true; btn.textContent = _t(_t(window.t('збереження'),window.t('сохранение')),'Сохранение...'); }
  try {
    ...
    <div style="background:#fff;border-radius:14px;width:100%;max-width:400px;box-shadow:0 20px 60px rgba(0,0,0,0.2);">
      ...
      <div style="padding:1.1rem 1.2rem;display:flex;flex-direction:column;gap:0.85rem;">
        ...
        <div>
          ...
          <input id="wpEvLabel" type="text" placeholder="${_t(window.t('напрСезоннийБонусВелика'),window.t('напрСезонныйБонусКрупная'))}"
