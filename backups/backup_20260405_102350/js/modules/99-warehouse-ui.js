...
(function () {
      ...
      <div style="display:flex;flex-direction:column;gap:0.75rem;">
        ...
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
          <div style="position:relative;flex:1;min-width:180px;">
            ...
            <input type="text" placeholder="${_t(window.t('пошук'),window.t('поиск'))}" value="${_searchQuery}"
    ...
    return `
      <div style="background:white;border-radius:12px;box-shadow:0 1px 4px rgba(0,0,0,0.06);overflow:hidden;">
        ...
        <div style="padding:0.75rem 1rem;display:flex;gap:0.75rem;align-items:center;border-bottom:1px solid #f3f4f6;flex-wrap:wrap;">
          <input
            type="text"
            placeholder="${_t(window.t('пошукТовару'),window.t('поискТовара'))}"
          ...
          <select onchange="window._whByLocFilter(this.value)"
            ...
            <option value="">${_t(window.t('всіЛокації'),window.t('всеЛокации'))}</option>
      ...
      <div style="display:flex;flex-direction:column;gap:0.75rem;">
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
          ...
          <select onchange="window._whOpDateFilter(this.value)"
            style="padding:0.45rem 0.75rem;border:1px solid #e5e7eb;border-radius:8px;font-size:0.85rem;background:white;">
            <option value="">${_t(window.t('весьЧас'),window.t('всёВремя'))}</option>
        ...
        <div style="display:flex;flex-direction:column;gap:0.65rem;">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;">
            <div>
              ...
              <input id="wh_name" value="${item.name || ''}" style="${_inp()}" placeholder="${_t(window.t('назваТовару'),window.t('названиеТовара'))}">
            </div>
            <div>
          ...
          <div>
            ...
            <textarea id="wh_desc" style="${_inp()}height:56px;resize:none;" placeholder="${_t(window.t('додатковийОпис'),window.t('дополнительноеОписание'))}">${_whEscHtml(item.description || '')}</textarea>
          </div>
        </div>
  ...
  window._whOpItemChange = function (itemId, type) {
    ...
    if (info) {
      ...
      info.innerHTML = `${_t(window.t('поточнийЗалишок'),window.t('текущийОстаток'))} <b>${s.qty}</b> ${item?.unit || 'шт'} | ${_t(window.t('доступно'),'Доступно:')} <b>${s.available}</b>`;
    }
    ...
    _showModal(`
      <div style="padding:1.25rem;">
        ...
        <div style="display:flex;flex-direction:column;gap:0.65rem;">
          <div>
            <label style="font-size:0.78rem;color:#6b7280;">${_t('Назва *','Название *')}</label>
            <input id="wh_sup_name" value="${s.name || ''}" style="${_inp()}" placeholder="${_t(window.t('товПостачальник'),window.t('оооПоставщик'))}">
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;">
            <div>
    ...
    _showModal(`
      <div style="padding:1.25rem;">
        ...
        <div style="display:flex;flex-direction:column;gap:0.65rem;">
          <div>
            <label style="font-size:0.78rem;color:#6b7280;">${_t('Назва *','Название *')}</label>
            <input id="wh_loc_name" value="${l.name || ''}" style="${_inp()}" placeholder="${_t(window.t('головнийСклад'),window.t('главныйСклад'))}">
          </div>
          <div>
      ...
      <div style="padding:0.85rem 1rem;background:white;border-radius:12px;box-shadow:0 1px 4px rgba(0,0,0,0.06);margin-bottom:1rem;display:flex;gap:0.75rem;flex-wrap:wrap;align-items:center;">
        ...
        <div style="display:flex;gap:0.5rem;margin-left:auto;flex-wrap:wrap;align-items:center;">
          ...
          <select onchange="window._whRepSetItem(this.value)" style="padding:0.35rem 0.6rem;border:1px solid #e5e7eb;border-radius:8px;font-size:0.82rem;outline:none;color:#374151;max-width:180px;">
            <option value="">${_t(window.t('всіТовари'),window.t('всеТовары'))}</option>
