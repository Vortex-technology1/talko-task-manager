...
(function () {
  ...
  function renderOrderRow(o, i) {
    ...
    if (canManage()) {
      if (o.status === 'new') {
        actions.push(`<button onclick="window._soConfirm('${o.id}')" title="${tg(window.t('підтвердити1'),'Confirm')}" style="padding:4px 8px;border:none;border-radius:5px;cursor:pointer;font-size:.75rem;font-weight:600;background:#dbeafe;color:#2563eb">${tg('Підтвердити','Confirm')}</button>`);
      }
      if (o.status === 'new' || o.status === 'confirmed') {
        actions.push(`<button onclick="window._soCreateRealization('${o.id}')" title="${tg(window.t('реалізація'),'Realization')}" style="padding:4px 8px;border:none;border-radius:5px;cursor:pointer;font-size:.75rem;font-weight:600;background:#d1fae5;color:#059669">${tg('Реалізація','Realize')}</button>`);
      }
  ...
  function updateTotals() {
    ...
    if (hint) {
      if (autoDisc > 0 && discount === 0) {
        ...
        hint.style.display = 'block';
        hint.textContent = `✅ ${tg(window.t('знижка1'),'Discount')} ${autoDisc}% ${tg(window.t('застосована'),'applied')}`;
