...
└──→ ПРАВА ДОСТУПА — роль (Владелец/Менеджер/Сотрудник) = набор разрешений</pre>
        ...
        function renderUsers() {
            ...
            c.innerHTML = visibleUsers.map(u => {
                ...
                const tgOk = !!u.telegramChatId;
                const gcOk = !!u.googleRefreshToken;
                const tgColor = tgOk ? '#22c55e' : '#e5e7eb';
                const gcColor = gcOk ? '#22c55e' : '#e5e7eb';
                const tgTitle = tgOk ? (window.t('telegramConnected') || 'Telegram підключено') : (window.t('telegramNotConnected') || 'Telegram не підключено');
                const gcTitle = gcOk ? (window.t('googleCalConnected') || 'Google Calendar підключено') : (window.t('googleCalNotConnected') || 'Google Calendar не підключено');

                const integrationBadges = `
                    <div style="display:flex;gap:4px;align-items:center;flex-shrink:0;" onclick="event.stopPropagation()">
                        <span title="${tgTitle}" style="display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:20px;font-size:0.68rem;font-weight:600;background:${tgOk ? '#dcfce7' : '#f3f4f6'};color:${tgOk ? '#16a34a' : '#9ca3af'};border:1px solid ${tgOk ? '#bbf7d0' : '#e5e7eb'};">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m22 2-7 20-4-9-9-4 20-7z"/><path d="M22 2 11 13"/></svg>
                            TG
                        </span>
                        <span title="${gcTitle}" style="display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:20px;font-size:0.68rem;font-weight:600;background:${gcOk ? '#dcfce7' : '#f3f4f6'};color:${gcOk ? '#16a34a' : '#9ca3af'};border:1px solid ${gcOk ? '#bbf7d0' : '#e5e7eb'};">
                ...
                <div class="user-card" style="cursor:pointer;" onclick="toggleUserDetail('${u.id}', event)">
                    <div style="display:flex;align-items:flex-start;gap:0.5rem;margin-bottom:0.3rem;">
                        <div style="flex:1;min-width:0;">
                            <div class="user-name" style="margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(u.name || u.email)}</div>
                            <div style="font-size:0.75rem;color:#6b7280;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${canViewEmails ? esc(u.email) : '●●●@●●●●●●'}${u.position ? ' · ' + esc(u.position) : ''}</div>
                            <div style="margin-top:4px;">${integrationBadges}</div>
                        </div>
                        <span class="role-badge ${u.role}" style="flex-shrink:0;margin-top:2px;">${getRoleText(u.role)}</span>
                    </div>
        ...
        async function openUserModal(userId = null) {
            ...
            const ALL_MODULES = [
                ...
                { key: 'coordination', label: 'Координація' },
