// ============================================================
// 42-niche-furniture.js — Меблевий бізнес
...
function _furnitureDemoDate(offsetDays) {
...
window._DEMO_NICHE_MAP['furniture_factory'] = async function() {
    ...
    const TASKS = [
        ...
        { t:'Перевірити KPI виробництва — виконання замовлень',          fi:7, ai:0, st:'new',      pr:'high',   d:0,  tm:'09:00', est:30,  r:'KPI зафіксовані, затримки визначені' },
        { t:'Узгодити ціну з постачальником ПВХ-плівок',                fi:5, ai:0, st:'new',      pr:'high',   d:0,  tm:'11:00', est:30,  r:'Нова ціна погоджена або альтернатива знайдена' },
        { t:'Зустріч з великим клієнтом — офіс 20 робочих місць',       fi:1, ai:0, st:'new',      pr:'high',   d:0,  tm:'14:00', est:60,  r:'ТЗ отримано, КП заплановано' },
        { t:'Контроль якості замовлення №МБ-087 перед відвантаженням',  fi:3, ai:0, st:'progress', pr:'high',   d:0,  tm:'16:00', est:30,  r:'Меблі перевірені, дефекти усунені або зафіксовані' },
        { t:'Підписати акт здачі кухні — Ковалі',                       fi:1, ai:0, st:'new',      pr:'medium', d:-1, tm:'18:00', est:30,  r:'Акт підписаний, відгук отримано' },
        // Сьогодні — команда
    ...
    for (const t of TASKS) {
        ...
        ops.push({type:'set', ref, data:{
            ...
            deadlineDate:_furnitureDemoDate(t.d), deadlineTime:t.tm,
    ...
    for (const r of REGS) {
        ...
        ops.push({type:'set', ref:cr.collection('regularTasks').doc(), data:{
            ...
            timeEnd:         timeEnd,
            duration:        r.est || 30,
            functionId:      fRefs[r.fi].id,
            functionName:    FUNCS[r.fi].name,
            assigneeId:      sRefs[r.ai].id,
            assigneeName:    STAFF[r.ai].name,
            creatorId:       uid,
            creatorName:     STAFF[0].name,
            estimatedTime:   String(r.est || 30),
    ...
    for (const p of PROCS) {
        ...
        ops.push({type:'set', ref:cr.collection('processes').doc(), data:{
            ...
            startDate:    _furnitureDemoDate(-7),
            deadline:     _furnitureDemoDate(21),
            createdBy:    uid,
            createdAt:    now,
            updatedAt:    now,
        }});
    ...
    for (const p of PROJS) {
        ops.push({type:'set', ref:cr.collection('projects').doc(), data:{
            ...
            startDate:_furnitureDemoDate(p.start), deadline:_furnitureDemoDate(p.end),
    ...
    for (const proj of projDocs) {
        ...
        if (pn.includes('шоурум') || pn.includes('Шоурум') || pn.includes('Сагайдачного')) {
            stages = [
                {name:'Ремонт та підготовка приміщення', status:'done',       order:1, start:_furnitureDemoDate(-30), end:_furnitureDemoDate(-10)},
                {name:'Закупівля обладнання та меблів',  status:'in_progress',order:2, start:_furnitureDemoDate(-10), end:_furnitureDemoDate(10) },
                {name:'Оформлення та розстановка',       status:'planned',    order:3, start:_furnitureDemoDate(10),  end:_furnitureDemoDate(30) },
                {name:'Відкриття та маркетинг',          status:'planned',    order:4, start:_furnitureDemoDate(35),  end:_furnitureDemoDate(45) },
            ];
        } else if (pn.includes('ІТ Хаб') || pn.includes('іт хаб') || pn.includes('Дмитренко')) {
            stages = [
                {name:'Погодження ТЗ та договір',        status:'done',       order:1, start:_furnitureDemoDate(-14), end:_furnitureDemoDate(-12)},
                {name:'Закупівля матеріалів',             status:'done',       order:2, start:_furnitureDemoDate(-12), end:_furnitureDemoDate(-8) },
                {name:'Виробництво партія 1 (6 столів)', status:'in_progress',order:3, start:_furnitureDemoDate(-5),  end:_furnitureDemoDate(4)  },
                {name:'Виробництво партія 2 (6 столів)', status:'planned',    order:4, start:_furnitureDemoDate(5),   end:_furnitureDemoDate(12) },
                {name:'Доставка та монтаж',               status:'planned',    order:5, start:_furnitureDemoDate(13),  end:_furnitureDemoDate(15) },
                {name:'Здача та оплата залишку',          status:'planned',    order:6, start:_furnitureDemoDate(15),  end:_furnitureDemoDate(17) },
            ];
        } else if (pn.includes('Еко') || pn.includes('еко') || pn.includes('Дерево')) {
            stages = [
                {name:'Дизайн та прототипи',             status:'in_progress',order:1, start:_furnitureDemoDate(-60), end:_furnitureDemoDate(-20)},
                {name:'Виготовлення зразків (3 моделі)', status:'planned',    order:2, start:_furnitureDemoDate(-10), end:_furnitureDemoDate(20) },
                {name:'Тестування та доопрацювання',     status:'planned',    order:3, start:_furnitureDemoDate(20),  end:_furnitureDemoDate(50) },
                {name:'Фото та запуск продажів',         status:'planned',    order:4, start:_furnitureDemoDate(55),  end:_furnitureDemoDate(90) },
            ];
        }
    ...
    DEALS.forEach((d, i) => {
        clientOps.push({type:'set', ref:clientRefs[i], data:{
            ...
            source:    d.src,
            niche:     'furniture',
            createdAt: _furnitureTs(-_ages[Math.floor(Math.random()*_ages.length)]),
            updatedAt: now,
        }});
    });
    ...
    DEALS.forEach((d, i) => {
        dealOps.push({type:'set', ref:cr.collection('crm_deals').doc(), data:{
            ...
            amount:          d.amt,
            note:            d.note,
            nextContactDate: d.nc !== null ? _furnitureDemoDate(d.nc) : null,
    ...
    for (const op of whOpDefs) {
        whOpRecs.push({type:'set', ref:cr.collection('warehouse_operations').doc(), data:{
            ...
            note: op.note,
            date: _furnitureDemoDate(op.d),
            createdBy: uid, createdAt: _furnitureTs(op.d),
        }});
    }
    ...
    const apptDefs = [
        { name:'Коваль Петро',    phone:'+380671234001', email:'koval@gmail.com',   date:_furnitureDemoDate(1),  time:'10:00', status:'confirmed', note:'Кухня 4м, стиль мінімалізм' },
        { name:'Бойко Олена',     phone:'+380671234006', email:'boyko@gmail.com',   date:_furnitureDemoDate(2),  time:'14:00', status:'confirmed', note:'Дитяча кімната, дівчинка 8 років' },
        { name:'Романова Юлія',   phone:'+380671234007', email:'romanova@ukr.net',  date:_furnitureDemoDate(3),  time:'11:00', status:'pending',   note:'Кухня + вітальня, бюджет ~120 000' },
        { name:'Мельник Андрій',  phone:'+380671234008', email:'melnyk@gmail.com',  date:_furnitureDemoDate(4),  time:'16:00', status:'pending',   note:'Шафа в передпокій' },
        { name:'Тарасенко Ірина', phone:'+380671234020', email:'tarасенко@ukr.net', date:_furnitureDemoDate(-3), time:'10:00', status:'confirmed', note:'Спальня — ліжко + шафа' },
        { name:'Лисенко Олег',    phone:'+380671234021', email:'lysenko@gmail.com', date:_furnitureDemoDate(-7), time:'15:00', status:'confirmed', note:'Офіс 4 робочих місця' },
    ];
    const apptOps = [];
    for (const a of apptDefs) {
        apptOps.push({type:'set', ref:cr.collection('booking_appointments').doc(), data:{
            ...
            createdAt:   _furnitureTs(-Math.floor(Math.random()*14)),
            updatedAt:   now,
        }});
    }
    ...
    newTplOps.push({type:'set', ref:cr.collection('processes').doc(), data:{
        ...
        startDate:_furnitureDemoDate(-2), deadline:_furnitureDemoDate(5),
        createdBy:uid, createdAt:now, updatedAt:now,
    }});
    newTplOps.push({type:'set', ref:cr.collection('processes').doc(), data:{
        ...
        startDate:_furnitureDemoDate(0), deadline:_furnitureDemoDate(7),
        createdBy:uid, createdAt:now, updatedAt:now,
    }});
    newTplOps.push({type:'set', ref:cr.collection('processes').doc(), data:{
        ...
        startDate:_furnitureDemoDate(-5), deadline:_furnitureDemoDate(20),
        createdBy:uid, createdAt:now, updatedAt:now,
    }});
    ...
    if (_pByName.showroom) {
        ...
        for (const t of showroomTasks) {
            projTaskOps.push({type:'set', ref:cr.collection('tasks').doc(), data:{
                ...
                deadlineDate:_furnitureDemoDate(t.d), deadlineTime:'18:00',
                estimatedTime:String(t.est), expectedResult:t.r,
    ...
    const budgetMonths = [
        { month: _furnitureDemoDate(0).slice(0,7), goal:320000 }, // поточний місяць
        { month: _furnitureDemoDate(-30).slice(0,7), goal:290000 }, // минулий
        { month: _furnitureDemoDate(30).slice(0,7), goal:350000 }, // наступний
    ];
    const budgetOps = [];
    ...
    const curDay   = _furnitureDemoDate(0);
    ...
    if (locIds.length > 0 && itemData.length > 0) {
        const invMonth = _furnitureDemoDate(-15).slice(0,7); // минулий місяць
        ...
        await window.safeBatchCommit([{type:'set', ref:cr.collection('warehouse_inventories').doc(), data:{
            ...
            status:     'confirmed',
            createdBy:  uid,
            createdAt:  _furnitureTs(-15),
            updatedAt:  _furnitureTs(-15),
        }}]);
    }
    ...
    const todayClientOps = todayDeals.map((d, i) => ({type:'set', ref:todayClientRefs[i], data:{
        ...
        createdAt: _furnitureTs(-1), updatedAt: now,
    }}));
    ...
    todayDeals.forEach((d, i) => {
        todayDealOps.push({type:'set', ref:cr.collection('crm_deals').doc(), data:{
            ...
            amount:          d.amt,
            note:            d.note,
            nextContactDate: _furnitureDemoDate(d.nc),
            nextContactTime: d.nc === 0 ? '14:00' : '10:00',
            assigneeId:      sRefs[1].id,
            assigneeName:    STAFF[1].name,
            createdAt:       _furnitureTs(-1),
            updatedAt:       now,
        }});
    });
...
if (window._NICHE_LABELS) window._NICHE_LABELS['furniture_factory'] = 'Меблевий бізнес';
