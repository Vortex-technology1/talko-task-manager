...
<div class="bk-cal-card">
  ...
  <div class="bk-cal-card-actions">
    ...
    <button class="bk-btn-sm bk-btn-toggle"
            ...
            title="${isActive ? (t('disableWord')||'') : (t('enableWord')||'')}">
...
<div class="bk-grp-card">
  ...
  <div class="bk-cal-card-actions">
    ...
    <button class="bk-btn-sm" data-bk-action="grp-delete" data-id="${grp.id}" data-name="${esc(grp.name)}"
            title="${t('flowDelete')||''}" style="color:#ef4444">
      ${I.trash}
    </button>
  </div>
</div>`;
...
window._bkGrpUpdateCount = function() {
    ...
    if (el) {
        el.textContent = n > 0 ? `${n} ${t('bkCalendarsInGroup')||window.t('')}` : '';
...
window._bkOpenApptModal = async function(apptId) {
    ...
    const STATUS_LABELS = { confirmed:`✅ ${t('confirmedWord3')||''}`, pending:`⏳ ${t('pendingWord3')||''}`, cancelled:`❌ ${t('cancelledWord')||''}`, completed:`✔ ${t('completedWord')||''}` };
...
<div class="bk-header-row">
  ...
  <select class="bk-select-sm" id="bk-appt-status-filter"
    ...
    <option value="pending">${t('pendingWord3')||''}</option>
    <option value="confirmed">${t('confirmedWord3')||''}</option>
    <option value="cancelled">${t('cancelledWord')||''}</option>
    <option value="completed">${t('completedWord')||''}</option>
  </select>
</div>
...
window._bkSaveCalendar = async function() {
    ...
    if (!slug) { alert(''); slugEl.focus(); return; }
    if (!/^[a-z0-9-]+$/.test(slug)) { alert(''); return; }
    const btn = document.getElementById('bk-save-btn');
    if (btn) { btn.disabled = true; btn.textContent = t('savingDots')||''; }
