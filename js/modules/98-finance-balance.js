    ...
    <div style="${colStyle}">
      ...
      ${d.assets.fixed.items.length
        ...
        : `<div style="padding:8px 14px;font-size:0.75rem;color:#9ca3af;">
            Обладнання, авто, нерухомість.
            <a href="#" onclick="window._showBalanceSettings();return false;" style="color:#3b82f6;">Додати →</a>
          </div>`}

      <div style="background:#f0fdf4;padding:10px 14px;display:flex;justify-content:space-between;border-top:2px solid #22c55e;">
        <span style="font-size:0.82rem;font-weight:700;color:#16a34a;">РАЗОМ АКТИВИ</span>
    ...
    <div style="display:flex;flex-direction:column;gap:12px;">
      ...
      <div style="${colStyle}">
        ...
        ${d.liabilities.current.items.length
          ...
          : emptyRow('Немає поточних зобов\'язань')}

        ${subHdr('Довгострокові зобов\'язання', d.liabilities.longTerm.total)}
        ...
        <div style="background:#fef2f2;padding:10px 14px;display:flex;justify-content:space-between;border-top:2px solid #ef4444;">
          <span style="font-size:0.82rem;font-weight:700;color:#dc2626;">РАЗОМ ПАСИВИ</span>
      ...
      <div style="${colStyle}">
        ...
        <div style="background:#eff6ff;padding:10px 14px;display:flex;justify-content:space-between;border-top:2px solid #3b82f6;">
          <span style="font-size:0.82rem;font-weight:700;color:#2563eb;">РАЗОМ КАПІТАЛ</span>
  ...
  const hints = `
    <div style="margin-top:14px;display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:10px;">
      <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:10px 14px;font-size:0.75rem;color:#1e40af;">
        <b>Дебіторка</b> формується автоматично з CRM (угоди «Виграно» без оплати).
        Щоб дебіторка зникла — зафіксуйте оплату в CRM або в транзакції.
      </div>
      <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:10px 14px;font-size:0.75rem;color:#166534;">
        <b>Запаси</b> підтягуються автоматично зі Складу (кількість × середня ціна).
        Для точності — регулярно оновлюйте ціни у картках товарів.
      </div>
...
window._showBalanceSettings = function() {
  ...
  const listHtml = (items, prefix) => items.map((it,i)=>`
    <div style="display:flex;gap:6px;align-items:center;margin-bottom:6px;">
      <input type="text" value="${_esc(it.name||'')}" placeholder="Назва"
        id="${prefix}_name_${i}"
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:14px;width:100%;max-width:520px;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.2);">
      ...
      <div style="padding:1.1rem 1.25rem;display:flex;flex-direction:column;gap:1rem;">
        ...
        <div>
          <label style="font-size:0.78rem;font-weight:600;color:#374151;display:block;margin-bottom:4px;">Статутний капітал</label>
        ...
        <div>
          <div style="font-size:0.78rem;font-weight:600;color:#374151;margin-bottom:6px;">
            Необоротні активи (обладнання, авто, нерухомість)
        ...
        <div>
          <div style="font-size:0.78rem;font-weight:600;color:#374151;margin-bottom:6px;">
            Поточні зобов'язання (заборгованість постачальникам)
        ...
        <div style="display:flex;gap:8px;margin-top:4px;">
          <button onclick="document.getElementById('balanceSettingsModal')?.remove()"
            style="flex:1;padding:0.55rem;border:1px solid #e5e7eb;border-radius:8px;background:#fff;cursor:pointer;font-size:0.83rem;color:#6b7280;">Скасувати</button>
          <button onclick="window._bsSave()" id="bsSaveBtn"
            style="flex:2;padding:0.55rem;border:none;border-radius:8px;background:#22c55e;color:#fff;cursor:pointer;font-size:0.83rem;font-weight:700;">Зберегти</button>
        </div>
      </div>
    </div>`;
...
window._bsAddRow = function(prefix) {
  ...
  div.innerHTML = `
    <input type="text" placeholder="Назва" id="${prefix}_name_${i}"
      style="flex:2;padding:5px 8px;border:1px solid #e5e7eb;border-radius:6px;font-size:0.8rem;">
