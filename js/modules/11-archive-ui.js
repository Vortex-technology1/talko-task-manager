...
'use strict';
        ...
        window.autoGenerateRegularTasks = async function autoGenerateRegularTasks() {
            ...
            // Skip якщо вже згенеровано сьогодні для цього пристрою
            const autoGenKey = `autoGen_${currentCompany}_${currentUser.uid}`;
            if (localStorage.getItem(autoGenKey) === todayStr) return;

            // БАГ M1 fix: Distributed lock через Firestore
            // Атомарний запис lock-документа — якщо вже є, інший пристрій вже генерує
            const lockRef = db.collection('companies').doc(currentCompany)
                .collection('_autogen_locks').doc(`${todayStr}_${currentUser.uid}`);
            try {
                await db.runTransaction(async (tx) => {
                    const lockDoc = await tx.get(lockRef);
                    if (lockDoc.exists) throw new Error('ALREADY_LOCKED');
                    tx.set(lockRef, {
                        date: todayStr,
                        uid: currentUser.uid,
                ...
                if (lockErr.message === 'ALREADY_LOCKED') {
                    // Інший пристрій вже генерує — встановлюємо localStorage щоб не питати знову
                    localStorage.setItem(autoGenKey, todayStr);
                    return;
                }
                // Якщо транзакція впала з іншої причини — продовжуємо без lock
                console.warn('[AutoGenerate] Lock failed, proceeding without lock:', lockErr.message);
            }
            ...
            for (const rt of regularTasks) {
                // BUG-Q FIX: skip disabled or paused regular tasks
                if (rt.disabled || rt.paused || rt.status === 'paused' || rt.status === 'disabled') continue;
                ...
                for (const assigneeId of assigneeIds) {
                    ...
                    tasksToCreate.push({
                        ...
                        creatorName: currentUserData?.name || currentUser.email || window.t('systemUser'),
                        regularTaskId: rt.id,
            ...
            try {
                ...
                // Видаляємо lock при помилці — щоб наступна спроба могла пройти
