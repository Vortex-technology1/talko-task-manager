...
(function () {
  ...
  function renderOrderRow(o, i) {
    ...
    if (canManage()) {
      if (o.status === 'new') {
        actions.push(`<button onclick="window._soConfirm('${o.id}')" title="${tg('','Confirm')}" style="padding:4px 8px;border:none;border-radius:5px;cursor:pointer;font-size:.75rem;font-weight:600;background:#dbeafe;color:#2563eb">${tg('','Confirm')}</button>`);
      }
      if (o.status === 'new' || o.status === 'confirmed') {
        actions.push(`<button onclick="window._soCreateRealization('${o.id}')" title="${tg('','Realization')}" style="padding:4px 8px;border:none;border-radius:5px;cursor:pointer;font-size:.75rem;font-weight:600;background:#d1fae5;color:#059669">${tg('','Realize')}</button>`);
      }
      actions.push(`<button onclick="window.openSalesOrderModal('${o.id}')" title="${tg('','Edit')}" style="padding:4px 6px;border:1px solid #e5e7eb;border-radius:5px;cursor:pointer;background:#fff;color:#6b7280"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>`);
      if (o.status === 'new') {
        actions.push(`<button onclick="window._soCancel('${o.id}')" title="${tg('','Cancel')}" style="padding:4px 6px;border:1px solid #fecaca;border-radius:5px;cursor:pointer;background:#fff;color:#dc2626"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`);
      }
    }
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px">
              <div>
                ...
                <select id="soFldClient" class="so-inp" style="width:100%" onchange="window._soClientChange(this)">
                  <option value="">${tg('','— select client —')}</option>
                  ${clientOptions}
                </select>
              </div>
              <div>
            ...
            ${S.priceLists.length ? `<div style="display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center;margin-bottom:14px">
              <div>
                ...
                <select id="soFldPriceList" class="so-inp" style="width:100%" onchange="window._soPriceListChange(this)">
                  <option value="">${tg('','— base item prices —')}</option>
                  ${priceListOptions}
                </select>
              </div>
      ...
      return `<tr class="so-item-row" style="border-bottom:1px solid #f1f5f9" data-idx="${idx}">
        <td style="padding:6px 10px">
          <select class="so-inp" style="width:100%;font-size:.8rem" onchange="window._soItemWarehouse(${idx},this)">
            <option value="">${tg('','— or enter manually —')}</option>
            ${warehouseOptions}
          </select>
  ...
  function updateTotals() {
    ...
    if (hint) {
      if (autoDisc > 0 && discount === 0) {
        ...
        hint.textContent = `💡 ${tg('','Auto discount')} ${autoDisc}% ${tg('','available at current amount')} — ${tg('','apply to items')}`;
      } else if (autoDisc > 0) {
        hint.style.display = 'block';
        hint.textContent = `✅ ${tg('','Discount')} ${autoDisc}% ${tg(window.t(''),'applied')}`;
