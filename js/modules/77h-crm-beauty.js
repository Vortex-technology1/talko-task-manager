...
window._renderBeautyTab = async function(deal) {
    ...
    if (!content) return;
    content.innerHTML = '<div style="padding:2rem;text-align:center;color:#9ca3af;font-size:.82rem;">Завантаження beauty профілю...</div>';
    ...
    }).join('') : '<div style="font-size:.78rem;color:#9ca3af;padding:.5rem 0;">Записів ще немає</div>';
    ...
    <div style="padding:0 .25rem;">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.5rem;margin-bottom:1rem;">
            <div style="background:#f0fdf4;border-radius:10px;padding:.6rem;text-align:center;">
                ...
                <div style="font-size:.68rem;color:#525252;">Візитів</div>
            </div>
            ...
            <div style="background:#faf5ff;border-radius:10px;padding:.6rem;text-align:center;">
                ...
                <div style="font-size:.68rem;color:#525252;">Балів</div>
            </div>
        </div>
        ...
        <div style="background:#f8fafc;border-radius:8px;padding:.55rem .75rem;margin-bottom:1rem;font-size:.78rem;color:#525252;">
            <span style="font-weight:600;">Останній візит:</span> ${esc(client.lastVisitDate)}
        </div>` : ''}
        ...
        <!-- Історія візитів -->
        <div style="margin-bottom:.5rem;">
            <div style="font-weight:700;font-size:.82rem;color:#374151;margin-bottom:.5rem;">📋 Історія візитів</div>
...
window._loadClientSubsWidget = async function(clientId) {
    ...
    if (!subs.length) {
        el.innerHTML = `<div style="font-weight:700;font-size:.82rem;color:#374151;margin-bottom:.5rem;">🎟 Абонементи</div>
            <div style="font-size:.75rem;color:#9ca3af;padding:.5rem 0;">Активних абонементів немає</div>`;
        return;
    }
    const esc = s => String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
...
window._saveBeautyProfile = async function(clientId) {
    ...
    try {
        ...
        window.showToast?.('Beauty профіль збережено ✓', 'success');
    } catch(e) {
...
window._createBeautyClientProfile = async function(dealId) {
    ...
    try {
        ...
        window.showToast?.('Beauty профіль створено ✓', 'success');
