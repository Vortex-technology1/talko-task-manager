...
'use strict';
        ...
        window.renderFunctionsCanvas = function() {
            ...
            const cards = positioned.map(function(f) {
                ...
                return `<div class="func-canvas-card" data-func-id="${safeId}" style="position:absolute;left:${f.x}px;top:${f.y}px;width:${CARD_W}px;background:${load.bg};border:2px solid ${load.border};border-radius:10px;padding:0.55rem 0.65rem;cursor:grab;user-select:none;box-shadow:0 2px 6px rgba(0,0,0,0.07);${topBar}" onmousedown="startCanvasDrag(event,'${safeId}')" ondblclick="openFunctionModal('${safeId}')"><div style="font-weight:700;font-size:0.8rem;color:#111827;line-height:1.3;margin-bottom:2px;">${safeName}</div>${noOwner ? `<div style="font-size:0.7rem;color:#ef4444;margin-bottom:3px;display:flex;align-items:center;gap:3px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> Немає власника</div>` : `<div style="font-size:0.7rem;color:#6b7280;margin-bottom:3px;display:flex;align-items:center;gap:3px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> ${safeOwner}</div>`}${safeResult ? `<div style="font-size:0.68rem;color:#374151;font-style:italic;line-height:1.3;margin-bottom:4px;">${safeResult}</div>` : ''}<div style="display:flex;gap:8px;font-size:0.68rem;color:#6b7280;"><span style="display:flex;align-items:center;gap:2px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg> ${fTasks}</span>${overdue > 0 ? `<span style="color:#ef4444;font-weight:600;display:flex;align-items:center;gap:2px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2.5"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> ${overdue}</span>` : ''}</div></div>`;
            }).join('');
