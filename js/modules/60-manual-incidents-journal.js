// =====================
    ...
    function toggleAddIncidentForm() {
        ...
        <div style="background:#f9fafb;border-radius:10px;padding:1rem;border:1px solid #e5e7eb;">
            <div style="font-weight:600;font-size:0.9rem;margin-bottom:0.75rem;"><i data-lucide="flag" class="icon icon-sm" style="color:#ef4444;"></i> ${t('addIncident') || 'Зафіксувати збій'}</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;">
                <div style="grid-column:1/-1;">
                    <input type="text" id="incidentTitle" placeholder="${t('whatHappened') || 'Що сталось?'}" style="width:100%;padding:0.5rem;border:1px solid #e5e7eb;border-radius:8px;font-size:0.85rem;">
                </div>
                <div>
                    <select id="incidentCategory" style="width:100%;padding:0.5rem;border:1px solid #e5e7eb;border-radius:8px;font-size:0.85rem;">
                        <option value="people">${t('cat_people') || 'Люди'}</option>
                        <option value="process">${t('cat_process') || 'Процес'}</option>
                        <option value="finance">${t('cat_finance') || 'Фінанси'}</option>
                        <option value="clients">${t('cat_clients') || 'Клієнти'}</option>
                        <option value="quality">${t('cat_quality') || 'Якість'}</option>
                        <option value="other">${t('cat_other') || 'Інше'}</option>
                    </select>
                </div>
                <div>
                    <select id="incidentSeverity" style="width:100%;padding:0.5rem;border:1px solid #e5e7eb;border-radius:8px;font-size:0.85rem;">
                        <option value="1">${t('severityLow') || 'Незначний'}</option>
                        <option value="2" selected>${t('severityMedium') || 'Середній'}</option>
                        <option value="3">${t('severityHigh') || 'Критичний'}</option>
                    </select>
                </div>
                <div style="grid-column:1/-1;">
                    <select id="incidentResponsible" style="width:100%;padding:0.5rem;border:1px solid #e5e7eb;border-radius:8px;font-size:0.85rem;">
                        <option value="">— ${t('responsible') || 'Відповідальний'} —</option>
                        ${userOptions}
                    </select>
                </div>
                <div style="grid-column:1/-1;">
                    <textarea id="incidentDescription" placeholder="${t('incidentDetails') || 'Деталі, причина, що робити...'}" style="width:100%;padding:0.5rem;border:1px solid #e5e7eb;border-radius:8px;font-size:0.85rem;min-height:50px;resize:vertical;"></textarea>
                </div>
            </div>
            <div style="display:flex;gap:0.5rem;margin-top:0.75rem;">
                ...
                <button class="btn" onclick="toggleAddIncidentForm();">${t('cancel') || 'Скасувати'}</button>
            </div>
        </div>`;
    ...
    async function saveIncident() {
        ...
        if (!title) { alert(t('enterName') || 'Введіть опис'); return; }
        ...
        try {
            ...
            renderControl();
            showToast(t('incidentSaved') || 'Збій зафіксовано', 'success', 2000);
        } catch(e) {
    ...
    async function resolveIncident(id) {
        try {
            ...
            renderControl();
            showToast(t('incidentResolved') || 'Збій розібрано', 'success', 2000);
        } catch(e) {
