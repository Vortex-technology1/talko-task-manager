...
'use strict';
        ...
        async function saveTask(e) {
            ...
            try {
                ...
                // BUG 6 FIX: для нових завдань data.creatorId ще не встановлений (встановлюється нижче)
                // тому підставляємо currentUser.uid напряму
                const _reviewCreatorId = existingForReview ? existingForReview.creatorId : (currentUser?.uid || '');
                const taskForReviewCheck = existingForReview || { creatorId: _reviewCreatorId, assigneeId: data.assigneeId, requireReview: data.requireReview, autoGenerated: false };
                if (statusVal === 'done' && shouldSendForReview(taskForReviewCheck)) {
                    ...
                    data.sentForReviewAt = firebase.firestore.FieldValue.serverTimestamp();
                    data.completedAt = null;
                    data.completedDate = null; // FIX C: clear when reverting from done
                    data.completedBy = null;
                } else {
                    data.completedAt = null;
                    data.completedDate = null; // FIX C: clear completedDate when not done
                    data.completedBy = null;   // FIX C: clear completedBy when not done
                }
