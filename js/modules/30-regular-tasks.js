...
'use strict';
        ...
        function updateRegularTaskFunctions() {
            ...
            s.innerHTML = `<option value="">${window.t('select')}</option>` + activeFunctions.map(f => `<option value="${esc(f.name)}">${esc(f.name)} (${f.assigneeIds?.length || 0} ${window.t('people')})</option>`).join('');
        ...
        async function saveRegularTask(e) {
            ...
            if (!directAssignee && (!func || !func.assigneeIds?.length)) {
                showAlertModal(window.t('selectAssigneeOrFunction'));
                return;
            }
            ...
            if (period === 'weekly' && selectedDays.length === 0) {
                showAlertModal(window.t('selectAtLeastOneDay'));
                return;
            }
        ...
        async function generateFromRegular(id) {
            ...
            if (!func || !func.assigneeIds?.length) {
                showAlertModal(window.t('noExecutorsInFunction'));
                return;
            }
            ...
            let periodStart;
            if (rt.period === 'weekly') {
                // Понеділок поточного тижня — НЕ мутуємо now
                const day = now.getDay();
                const mondayOffset = now.getDate() - day + (day === 0 ? -6 : 1);
                periodStart = getLocalDateStr(new Date(now.getFullYear(), now.getMonth(), mondayOffset));
            ...
            assigneeIds.forEach(assigneeId => {
                ...
                batch.set(ref, {
                    ...
                    deadline: firebase.firestore.Timestamp.fromDate(new Date(deadlineDate + 'T' + (rt.timeStart || rt.time || '18:00') + ':00')),
                    ...
                    creatorName: currentUserData?.name || currentUser?.email || window.t('systemUser'), // BUG5 FIX: use real user name
                    regularTaskId: id,
                    autoGenerated: true,   // BUG 4 FIX: узгоджено з autoGenerateRegularTasks
            ...
            showAlertModal(`${window.t('createdLabel')} ${assigneeIds.length} ${window.t('tasksWord')}`);
            renderMyDay();
            refreshCurrentView();
        }
        ...
        async function deleteRegularTask(id) {
            ...
            const taskName = rt.title || window.t('regularTaskFallback');
        ...
        async function completeRegularTask(id) {
            ...
            // BUG 2 FIX: підтримуємо rt.assigneeId (прямий виконавець) як і autoGenerateRegularTasks
            let resolvedAssigneeIds;
            if (rt.assigneeId) {
                resolvedAssigneeIds = [rt.assigneeId];
            } else {
                const func = functions.find(f => f.name === rt.function);
                resolvedAssigneeIds = func?.assigneeIds || [];
            }
            ...
            // BUG 3 FIX: completedDate потрібен для аналітики
            const completedDate = today;
            const completedBy = currentUser?.uid || '';
            ...
            if (existingTasks.length > 0) {
                ...
                taskToUpdate.completedBy = completedBy;
            } else {
                ...
                const newTaskData = {
                    ...
                    deadlineDate: today,
                    deadlineTime: rt.timeStart || rt.time || '18:00',
                    deadline: firebase.firestore.Timestamp.fromDate(new Date(today + 'T' + (rt.timeStart || rt.time || '18:00') + ':00')),
                    expectedResult: rt.expectedResult || '',
                    reportFormat: rt.reportFormat || '',
                    description: rt.instruction || '',
                    status: 'done',
        ...
        function renderRegularTasks() {
            ...
            const pt = { weekly: window.t('weekly'), monthly: window.t('monthly'), quarterly: window.t('quarterly') };
            const days = {
                '0': window.t('daySun'),
                '1': window.t('dayMon'),
                '2': window.t('dayTue'),
            ...
            if (filtered.length === 0) {
                const dayFilter = document.getElementById('regularDayFilter')?.value;
                const noTasksMsg = dayFilter ? window.t('noRegularForDay') : window.t('noRegular');
                c.innerHTML = `<div class="empty-table"><h3>${noTasksMsg}</h3><p>${window.t('createRegular')}</p></div>`;
                return;
            }
                ...
                <table class="tasks-table">
                    <thead>
                        <tr>
                            ...
                            <th>${window.t('function')}</th>
                            <th>${window.t('schedule')}</th>
                            <th>${window.t('statusPeriod')}</th>
                            <th>${window.t('completion30d')}</th>
                            <th>${window.t('assignees')}</th>
            ...
            filtered.forEach(rt => {
                ...
                if (rt.period === 'daily') {
                    scheduleText = `${window.t('daily')}, ${displayTime}`;
                } else if (rt.period === 'weekly') {
                    ...
                    if (daysList.length === 7) {
                        ...
                        scheduleText = `${window.t('monFri')}, ${displayTime}`;
                    } else {
                        ...
                        <td>
                            <div class="action-btns">
                                ${statusInfo.status === 'notCreated' ? `
                                    <button class="action-btn" onclick="openTodayRegularTask('${escId(rt.id)}')" title="${window.t('openTask')}" style="background:#2196f3;color:white;"><i data-lucide="external-link" class="icon icon-sm"></i></button>
                                    <button class="action-btn" onclick="completeRegularTask('${escId(rt.id)}')" title="${window.t('markDone')}" style="background:#4caf50;color:white;"><i data-lucide="check" class="icon icon-sm"></i></button>
                                ` : statusInfo.status === 'inProgress' ? `
                                    <button class="action-btn" onclick="openTodayRegularTask('${escId(rt.id)}')" title="${window.t('openTask')}" style="background:#2196f3;color:white;"><i data-lucide="external-link" class="icon icon-sm"></i></button>
                                    <button class="action-btn" onclick="completeRegularTask('${escId(rt.id)}')" title="${window.t('markDone')}" style="background:#4caf50;color:white;"><i data-lucide="check" class="icon icon-sm"></i></button>
                                ` : `
                                    <button class="action-btn" onclick="openTodayRegularTask('${escId(rt.id)}')" title="${window.t('openTask')}" style="opacity:0.6;"><i data-lucide="external-link" class="icon icon-sm"></i></button>
                                `}
                                ${rt.instruction ? `<button class="action-btn" onclick="showInstruction('${escId(rt.id)}')" title="${window.t('instruction')}"><i data-lucide="book-open" class="icon icon-sm"></i></button>` : ''}
                                <button class="action-btn" onclick="openRegularTaskModal('${escId(rt.id)}')" title="${window.t('edit')}"><i data-lucide="pencil" class="icon icon-sm"></i></button>
                                <button class="action-btn" onclick="deleteRegularTask('${escId(rt.id)}')" title="${window.t('delete')}"><i data-lucide="trash-2" class="icon icon-sm"></i></button>
                            </div>
                        </td>
                    </tr>`;
            });
        ...
        function showInstruction(id) {
            ...
            if (rt && rt.instruction) {
                showAlertModal(`${window.t('instructionLabel')}:\n\n${rt.instruction}`);
            }
        }
