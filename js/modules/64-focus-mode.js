...
'use strict';
    ...
    function getFocusTasks() {
        ...
        // Разові — assignee або coExecutor
        tasks.filter(t => {
            const isAssignee   = t.assigneeId === currentUser?.uid;
            const isCoExecutor = (t.coExecutorIds || []).includes(currentUser?.uid); // FIX BL
            if (!isAssignee && !isCoExecutor) return false;
        ...
        try {
            if (task.type === 'regular') {
                ...
                if (task.generatedTaskId) {
                    const todayStrDone = getLocalDateStr();
                    await db.collection('companies').doc(currentCompany).collection('tasks').doc(task.generatedTaskId).update({
                        status: 'done',
                        completedDate: todayStrDone, // FIX BI: потрібен для статистики та дашбордів
                    ...
                    const ref = await db.collection('companies').doc(currentCompany).collection('tasks').add({
                        ...
                        pinned: false,
                        completedDate: todayStr, // FIX BI: потрібен для статистики та дашбордів
                        completedAt: firebase.firestore.FieldValue.serverTimestamp(),
                ...
                const completedFields = newStatus === 'done' ? {
                    completedDate: getLocalDateStr(), // FIX BK: потрібен для статистики
                    completedAt: firebase.firestore.FieldValue.serverTimestamp()
                } : {};
